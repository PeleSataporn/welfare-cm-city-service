...
public class MonthlyTotalJob {
    ...
    @Scheduled(cron = "0 0 0 L * ?") // Execute at midnight on the last day of the month = [ 1."0 0 0 28-31 * ?" ** 2. "0 0 0 L * ?" ]  ||  0 */2 * * * * => 2 minute
    public void executeMonthlyJob() throws InterruptedException {
        ...
        int sumDelete = Math.round(resultTotal.getSumStockAccumulate() - resultTotal.getSumStockValue());
        ...
        log.info(" getSumStockAccumulate = " + summaryAll.getSumStockAccumulate());  //resultTotal.getSumStockAccumulate()
