...
public class LoanLogic01Repository {
  ...
  public StringBuilder buildQuerySqlGetMaxNumber() {
    ...
    sql.append(" select COUNT(loan_no) as numberMax from loan WHERE loan_no is not null ");
    return sql;
  }
  ...
  public StringBuilder buildQueryGetLoanDetailByLoanId(Long loanId) {
    val sql = new StringBuilder();
    sql.append(" select loan_id from loan_detail WHERE loan_id = " + loanId);
    return sql;
  }
  ...
  public StringBuilder buildQueryGetLoanAndDetail() {
    val sql = new StringBuilder();
    sql.append(" select * from loan WHERE loan.active = TRUE ");
    return sql;
  }

  public List<LoanDto> getLoanAndDetail() {
    val sql = buildQueryGetLoanAndDetail();
    return jdbcTemplate.query(sql.toString(), new BeanPropertyRowMapper<>(LoanDto.class));
  }
}
