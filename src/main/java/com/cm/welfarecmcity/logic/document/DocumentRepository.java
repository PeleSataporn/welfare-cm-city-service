...
public class DocumentRepository {
  ...
  public StringBuilder buildQuerySqlV1Loan(Long loanId, String getMonthCurrent, String testHistory, String yearCurrent) {
    ...
    sql.append(
      ...
      " RIGHT JOIN loan_detail ON (loan_detail.loan_id = loan.id AND loan_detail.deleted = FALSE AND loan_detail.active = TRUE ) WHERE 1=1 "
    );
...
//      " LEFT JOIN loan ON employee.loan_id = loan.id LEFT JOIN loan_detail ON loan_detail.loan_id = loan.id "
  ...
  public StringBuilder buildQuerySqlV1LoanOfNull(DocumentReq req) {
    ...
    sql.append(
            " SELECT * " +
                    " FROM loan_detail WHERE deleted = FALSE AND active = TRUE "
    );
    sql.append(" and loan_id = ").append(req.getLoanId());
    //if (req.getLoanId() != null) {
        sql
                .append(" and loan_month = '")
                .append(req.getMonthCurrent())
